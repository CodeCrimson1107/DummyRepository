...
From (SELECT COUNT(TOTPREM) AS QUOTES, CO_Name as Company, SUM(TOTPREM) AS TOTAL_PREMIUM, RiskArea AS RISKAREA, P_Level AS P_LEVEL,
  ...
  WHERE (Rate_Yr*100+Rate_Mo) >= 201910 and LiabLimits101 >=15 AND LiabLimits101<=30 and LiabLimits301 >=10 AND LiabLimits301<=30 
...
From (SELECT COUNT(TOTPREM) AS QUOTES, CO_Name as Company, SUM(TOTPREM) AS TOTAL_PREMIUM,  RiskArea AS RISKAREA, P_Level AS P_LEVEL,
  ...
  WHERE (Rate_Yr*100+Rate_Mo) >= 201910 and LiabLimits101 >=25 AND LiabLimits101<=30 and LiabLimits301 >=10 AND LiabLimits301<=30 
  GROUP BY CO_Name, TOTPREM, RiskArea, P_Level, Rate_Yr , Rate_Mo,  [Profile], Term, ZipCode,
             CASE WHEN ( left(rank1co,4) <> left(co_name,4) ) then (TOTPREM - RANK1PR)
                     WHEN ( left(RANK2CO,4) <> left(co_name,4) ) then (TOTPREM - RANK2PR)
                     WHEN ( left(RANK3CO,4) <> left(co_name,4) ) then (TOTPREM - RANK3PR)
...
From (SELECT COUNT(TOTPREM) AS QUOTES, CO_Name as Company, SUM(TOTPREM) AS TOTAL_PREMIUM,  RiskArea AS RISKAREA, P_Level AS P_LEVEL,
  ...
  WHERE (Rate_Yr*100+Rate_Mo) >= 201910 and LiabLimits101 >=25 AND LiabLimits101<=30 and LiabLimits301 >=25 AND LiabLimits301<=30  
  GROUP BY CO_Name, TOTPREM, RiskArea, P_Level, Rate_Yr , Rate_Mo,  [Profile], Term,ZipCode,
                CASE WHEN ( left(rank1co,4) <> left(co_name,4) ) then (TOTPREM - RANK1PR)
                     WHEN ( left(RANK2CO,4) <> left(co_name,4) ) then (TOTPREM - RANK2PR)
                     WHEN ( left(RANK3CO,4) <> left(co_name,4) ) then (TOTPREM - RANK3PR)
...
From (SELECT COUNT(TOTPREM) AS QUOTES, CO_Name as Company, SUM(TOTPREM) AS TOTAL_PREMIUM,  RiskArea AS RISKAREA, P_Level AS P_LEVEL,
  ...
  WHERE (Rate_Yr*100+Rate_Mo) >= 201910 and LiabLimits101 >=25 AND LiabLimits101<=30 and LiabLimits301 >=15 AND LiabLimits301<=30 
...
From (SELECT COUNT(TOTPREM) AS QUOTES, CO_Name as Company, SUM(TOTPREM) AS TOTAL_PREMIUM,  RiskArea AS RISKAREA, P_Level AS P_LEVEL,
  ...
  WHERE (Rate_Yr*100+Rate_Mo) >= 201911 and LiabLimits301 >=25 AND LiabLimits301<=30 AND LIABLIMITS101=30 
  GROUP BY CO_Name, TOTPREM, RiskArea, P_Level, Rate_Yr , Rate_Mo, Term, ZipCode,
              CASE WHEN ( left(rank1co,4) <> left(co_name,4) ) then (TOTPREM - RANK1PR)
                     WHEN ( left(RANK2CO,4) <> left(co_name,4) ) then (TOTPREM - RANK2PR)
                     WHEN ( left(RANK3CO,4) <> left(co_name,4) ) then (TOTPREM - RANK3PR)
...
From (SELECT COUNT(TOTPREM) AS QUOTES, CO_Name as Company, SUM(TOTPREM) AS TOTAL_PREMIUM,  RiskArea AS RISKAREA, P_Level AS P_LEVEL,
  ...
  WHERE (Rate_Yr*100+Rate_Mo) >= 201910 and  LiabLimits101 >=25 AND LiabLimits101<=30 and LiabLimits301 >=20 AND LiabLimits301<=30 
  GROUP BY CO_Name, TOTPREM, RiskArea, P_Level, Rate_Yr , Rate_Mo,  Term,  ZipCode,
               CASE WHEN ( left(rank1co,4) <> left(co_name,4) ) then (TOTPREM - RANK1PR)
                     WHEN ( left(RANK2CO,4) <> left(co_name,4) ) then (TOTPREM - RANK2PR)
                     WHEN ( left(RANK3CO,4) <> left(co_name,4) ) then (TOTPREM - RANK3PR)
...
From (SELECT COUNT(TOTPREM) AS QUOTES, CO_Name as Company, SUM(TOTPREM) AS TOTAL_PREMIUM,  RiskArea AS RISKAREA, P_Level AS P_LEVEL,
  ...
  WHERE (Rate_Yr*100+Rate_Mo) >= 201910 and  LiabLimits101<=30 and LiabLimits301 >=10 AND LiabLimits301<=30
...
Select sum(Quotes) as Quote, Company, sum(TOTAL_PREMIUM) as Total_Premium,  RISKAREA, P_LEVEL, QUOTE_YEAR, QUOTE_MM, [Profile],  TERM, [State], LABEL,ZIPCODE, yyyymm1
...
From (SELECT COUNT(TOTPREM) AS QUOTES, COMPANY, SUM(TOTPREM) AS TOTAL_PREMIUM,  AREA AS RISKAREA, PLEVEL AS P_LEVEL, TRANSYY AS QUOTE_YEAR, TRANSMM AS QUOTE_MM,TransYY*100+TransMM AS yyyymm1, 
		[PROFILE],  TERM, 'SC' AS [STATE],zip AS ZIPCODE,
...
Select sum(Quotes) as Quote, Company, sum(TOTAL_PREMIUM) as Total_Premium, RISKAREA, P_LEVEL, QUOTE_YEAR, QUOTE_MM,  [Profile],  TERM, [State], LABEL,Zip,yyyymm1
...
Group by Company, RISKAREA, P_LEVEL, QUOTE_YEAR, QUOTE_MM, [Profile], TERM, [State], LABEL,Zip, yyyymm1
...
From (SELECT COUNT(TOTPREM) AS QUOTES, COMPANY, SUM(TOTPREM) AS TOTAL_PREMIUM, MARKET AS RISKAREA, PLEVEL AS P_LEVEL, TRANSYY AS QUOTE_YEAR, TRANSMM AS QUOTE_MM, 
		[PROFILE], '6' AS TERM, 'GA' AS [STATE],zip,
		CASE WHEN RANK1DIFF <=0   THEN 'WIN'
			 WHEN RANK1DIFF <=30  THEN '1_30'
			 WHEN RANK1DIFF <=60  THEN '31_60'
			 WHEN RANK1DIFF <=90  THEN '61_90'
...
From (SELECT COUNT(TOTPREM) AS QUOTES, CO_NAME AS COMPANY, SUM(TOTPREM) AS TOTAL_PREMIUM,  RISKAREA, P_LEVEL, 
		...
		[PROFILE],  TERM, 'TN' AS [STATE],ZipCode,
  ...
  where Rate_Yr*100 + Rate_Mo<=201909
  GROUP BY CO_NAME,  RISKAREA, P_LEVEL, RATE_YR , RATE_MO, ZipCode,
...
Group by Company,  RISKAREA, P_LEVEL, QUOTE_YEAR, QUOTE_MM,  [Profile], TERM, [State], LABEL,ZipCode, yyyymm1

UNION All

Select sum(Quotes) as Quote, Company, sum(TOTAL_PREMIUM) as Total_Premium, RISKAREA, P_LEVEL, QUOTE_YEAR, QUOTE_MM,  [Profile],  TERM, [State], LABEL,ZipCode, yyyymm1

From (SELECT COUNT(TOTPREM) AS QUOTES, CO_NAME AS COMPANY, SUM(TOTPREM) AS TOTAL_PREMIUM,  RISKAREA, P_LEVEL, ZipCode,
		RATE_YR AS QUOTE_YEAR, RATE_MO AS QUOTE_MM,  
  ...
  GROUP BY CO_NAME, CASE WHEN (TOTPREM - RANK1PR) <=0  THEN 'WIN'
			 ...
			 WHEN (TOTPREM - RANK1PR) <=90 THEN '61_90'
			 WHEN (TOTPREM - RANK1PR) <=120 THEN '91_120'
			 ELSE 'GE_121' END,  RISKAREA, P_LEVEL, RATE_YR , RATE_MO,  ZipCode, 
		[PROFILE], TERM
		) as a 
Group by Company, RISKAREA, P_LEVEL, QUOTE_YEAR, QUOTE_MM, [Profile], TERM, [State], LABEL,ZipCode, yyyymm1
...
From (SELECT COUNT(TOTPREM) AS QUOTES, CO_NAME AS COMPANY, SUM(TOTPREM) AS TOTAL_PREMIUM, RISKAREA, P_LEVEL, ZipCode,
  ...
  where Rate_Yr*100+Rate_Mo<=201909
  GROUP BY CO_NAME, TOTPREM,  RISKAREA, P_LEVEL, RATE_YR, RATE_MO, ZipCode,
		[PROFILE],  LABELS
	) as a 
...
From (SELECT COUNT(TOTPREM) AS QUOTES, CO_NAME AS COMPANY, SUM(TOTPREM) AS TOTAL_PREMIUM,  RISKAREA, P_LEVEL, ZipCode,
  ...
  GROUP BY CO_NAME, TOTPREM,  RISKAREA, P_LEVEL, RATE_MO, 
		[PROFILE],  LABELS, RATE_YR,ZipCode
	) as a
Group by Company,  RISKAREA, P_LEVEL, QUOTE_YEAR, QUOTE_MM, [Profile],  TERM, [State], LABEL,ZipCode, yyyymm1
...
From (SELECT COUNT(TOTPREM) AS QUOTES, CO_Name as Company, SUM(TOTPREM) AS TOTAL_PREMIUM,  b.Risk_Area AS RISKAREA, b.P_LEVEL AS P_LEVEL, Rate_Yr AS QUOTE_YEAR, Rate_Mo AS QUOTE_MM, 
		[PROFILE],  TERM, 'AL' AS [STATE],a.ZipCode as zipcode,
